/*
 ...
 atmospheric data using the DS18B20 temp sensor, the DHT22 humidity/temp sensor, 
 and the MPL3115A2 altitude/pressure/temperature sensor.
 Date, time and temperature functions also collected using a Chronodot RTC.
 ...
 Based on MPL3115A code by: A.Weiss, 7/17/2012, changes Nathan Seidle Sept 23rd, 2013 (SparkFun)
 And Adafruit's TFT Library
 ...
 V4, 2016\02\05
 Revise to use pseudo-clock (based on millis() ) in revised ChronodotI2C code if 
 hardware Chronodot clock not installed
...
{ 
  ...
  char stc[5], stf[5];
  String cmdNames[] = {"version","sample", "printout", "csv", "xmlstart", "xmlstop", "help", "restart", "unrecognized"};
  typedef enum cmds {vers=0, sample, cprintout, ccsv, cxmlstart, cxmlstop, chelp, crestart, noCmd};
  int i=0,j=0;
  String cmd;
...
//  Serial.print("gotCmd="); Serial.print(gotCmd?"true":"false"); Serial.print("\ttimedOut="); Serial.println(timedOut?"true":"false");  
  ...
  while (!timedOut && !gotCmd) {
    while (!(gotCmd=(Serial.available() > 0)) && !(timedOut=((millis()-marker) > UPDATE_FREQ))) {delay(1);};  // get response or timeout
    if (gotCmd) {                       //  we received a command: get and parse it
      ...
      switch (i) {
        ...
        case chelp:
          ...
          for (int j=(int) vers; j < ((int) noCmd); j++) {Serial.print(cmdNames[j]); Serial.print(" | ");};
          Serial.println("?");
          gotCmd = false;
          break;
	case crestart:
