...
Written by HDTodd, Bozeman Montana & Williston Vermont, August, 2015.
  ...
  v5.1  Revised January, 2018
        Add date-time setting to probe code, set from Station

  v5.0  Revised Summer, 2017
        Incorporate DS18 support

  v4.2  Revised Spring, 2017
        Finish modularizing code, prepare for DS18 incorporation

  v4.1  Revised Feb-Aug 2016 to modularize code
 
  v4.0  Revised Feb, 2016
        Clean up code: reduce duplication, use procedures to 
  ...
  v3.0  Revised January, 2016
        Add sqlite3 database storage as alternative to MySQL.
...
int main(int argc, char *argv[]) 
{
  ...
  unsigned char tBuf[50];          // buffer for setting time
...
*/
  ...
  // Set the time on the probe from our time here
  curtime = time(NULL);
  now = localtime(&curtime);
  time_len = strftime(tBuf, sizeof tBuf, "settime %Y-%m-%d %T\n", now);
  if (time_len)
    RS232_SendBuf(Uno.portNum,tBuf,time_len);
  else
    fprintf(stderr,"[%WS] Weatherstation can't set time with string '%s'\n", tBuf);
